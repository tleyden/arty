...
// Context7 Keys
const CONTEXT7_API_KEY = "VIBEMACHINE_CONTEXT7_API_KEY";
    ...
    const results = await Promise.allSettled(
      keys.map(async (key) => {
        try {
          ...
          log.warn(
            `⚠️ Failed to load ${key} during cache initialization`,
            {},
            error,
          );
          cache.set(key, null);
...
async function getCachedValue(key: string): Promise<string | null> {
  ...
  try {
    ...
    log.warn(
      `⚠️ SecureStore access failed for ${key}, returning null`,
      {},
      error,
    );
    cache.set(key, null);
...
export async function deleteApiKey(): Promise<void> {
  ...
  try {
    ...
    log.warn(
      "⚠️ Failed to delete from SecureStore (may not exist)",
      {},
      errorDetails,
    );
    throw secureStoreError;
...
export async function saveGDriveClientIdOverride(
  clientId: string,
): Promise<void> {
  ...
  try {
    ...
    log.error(
      "❌ SecureStore failed to save GDrive Client ID override",
      {},
      errorDetails,
    );
    throw secureStoreError;
...
export async function deleteGDriveClientIdOverride(): Promise<void> {
  ...
  try {
    ...
    log.info(
      "✅ GDrive Client ID override deleted from SecureStore and cache",
      {},
    );
  } catch (secureStoreError) {
    const errorDetails = {
...
export async function getGDriveClientId(): Promise<string | null> {
  ...
  if (envDefault) {
    log.info(
      "ℹ️ Using GDrive Client ID from .env (EXPO_PUBLIC_GOOGLE_API_CLIENT_ID)",
      {},
    );
  } else {
    log.debug(
      "🔍 No GDrive Client ID in override or .env - look for one in secure-storage",
      {},
    );
  }
  return envDefault;
...
export async function saveGDriveAccessToken(token: string): Promise<void> {
  ...
  try {
    ...
    log.error(
      "❌ SecureStore failed to save GDrive access token",
      {},
      errorDetails,
    );
    throw secureStoreError;
...
export async function setGDriveAccessToken(
  token: string,
  _expiresIn?: number,
): Promise<void> {
...
export async function saveGDriveRefreshToken(token: string): Promise<void> {
  ...
  try {
    ...
    log.error(
      "❌ SecureStore failed to save GDrive refresh token",
      {},
      errorDetails,
    );
    throw secureStoreError;
...
export async function deleteGDriveAccessToken(): Promise<void> {
  try {
    ...
    log.warn(
      "⚠️ Failed to delete GDrive access token from SecureStore (may not exist)",
      {},
      errorDetails,
    );
  }
...
export async function deleteGDriveRefreshToken(): Promise<void> {
  try {
    ...
    log.warn(
      "⚠️ Failed to delete GDrive refresh token from SecureStore (may not exist)",
      {},
      errorDetails,
    );
  }
...
export async function saveLogfireApiKey(apiKey: string): Promise<void> {
  ...
  try {
    ...
    log.info(
      "✅ Logfire API key saved to SecureStore and cache successfully",
      {},
    );
    ...
    log.error(
      "❌ SecureStore failed to save Logfire API key",
      {},
      errorDetails,
    );
    throw secureStoreError;
...
// Context7 API Key Functions

export async function saveContext7ApiKey(apiKey: string): Promise<void> {
  if (!apiKey || apiKey.length === 0) {
    throw new Error("Context7 API key must not be empty.");
  }

  log.info("🔄 Attempting to save Context7 API key...", {});

  try {
    await setCachedValue(CONTEXT7_API_KEY, apiKey);
    log.info(
      "✅ Context7 API key saved to SecureStore and cache successfully",
      {},
    );

    const verification = await getCachedValue(CONTEXT7_API_KEY);
    if (verification) {
      log.info("✅ SecureStore verification successful", {});
    } else {
      log.error("❌ SecureStore verification failed", {});
    ...
    log.error(
      "❌ SecureStore failed to save Context7 API key",
      {},
      errorDetails,
    );
    throw secureStoreError;
...
export async function getContext7ApiKey(): Promise<string | null> {
  log.info("🔄 Attempting to retrieve Context7 API key...", {});

  try {
    const apiKey = await getCachedValue(CONTEXT7_API_KEY);

    if (apiKey) {
      log.info("✅ Context7 API key retrieved from cache", {});
      return apiKey;
    }

    log.info("ℹ️ No Context7 API key found in cache", {});
    return null;
  } catch (error) {
    const errorDetails = {
      name: (error as Error).name,
    ...
    log.error("❌ Failed to retrieve Context7 API key", {}, errorDetails);
    return null;
  }
}

export async function deleteContext7ApiKey(): Promise<void> {
  log.info("🔄 Attempting to delete Context7 API key from SecureStore...", {});

  try {
    await deleteCachedValue(CONTEXT7_API_KEY);
    log.info("✅ Context7 API key deleted from SecureStore and cache", {});
  } catch (secureStoreError) {
    const errorDetails = {
      name: (secureStoreError as Error).name,
      message: (secureStoreError as Error).message,
...
export async function hasContext7ApiKey(): Promise<boolean> {
  try {
    ...
    log.error(
      "❌ Failed to check Context7 API key existence",
      {},
      errorDetails,
    );
    return false;
...
export async function clearAllStoredSecrets(): Promise<void> {
  ...
  try {
    ...
    log.info(
      "📋 About to delete the following SecureStore keys:",
      {},
      {
    ...
    log.info(
      "📋 About to delete the following AsyncStorage keys:",
      {},
      {
    ...
    await Promise.all(
      secureStoreKeys.map((key) =>
        deleteCachedValue(key).catch((error) => {
          log.warn(
            `⚠️ Failed to delete SecureStore key: ${key}`,
            {},
            {
    ...
    await Promise.all(
      asyncStorageKeys.map((key) =>
        AsyncStorage.removeItem(key).catch((error) => {
          log.warn(
            `⚠️ Failed to delete AsyncStorage key: ${key}`,
            {},
            {
              error: (error as Error).message,
            },
    ...
    log.info(
      "✅ All stored secrets cleared successfully",
      {},
      {
